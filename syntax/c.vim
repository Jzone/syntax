...
"
"
hi Normal guifg=palegreen

syn match cUpper '[A-Z]'
hi cUpper guifg=yellowgreen

hi cComment	guifg=#5d959F gui=italic
hi hiConstant term=bold ctermfg=3 guifg=SkyBlue
hi def link cNumber hiConstant
hi def link cConstant	hiConstant
hi def link cCommentString hiConstant	
hi def link cComment2String hiConstant	
hi def link cString hiConstant	
syn match cConstant '[_A-Z]\{2,}'

hi cCmd guifg=indianred gui=italic,underline,bold
syn match cCmdEnd ';'
hi def link cCmdEnd cCmd
hi def link cConditional cCmd
hi def link cStatement cCmd
hi def link cLabel cCmd
hi def link cUserLabel	cCmd
hi def link cRepeat cCmd
...
if exists("c_curly_error")
  syn match cCurlyError "}"
  syn region	cBlock		matchgroup=cCmd start="{" end="}" contains=ALLBUT,cBadBlock,cCurlyError,@cParenGroup,cErrInParen,cCppParen,cErrInBracket,cCppBracket,@cStringGroup,@Spell fold
else
  syn region	cBlock		matchgroup=cCmd start="{" end="}" transparent fold
endif
...
syn region cFunStart matchgroup=cFun start="[A-Za-z_]\{1,}(" end=")" contains=cFunStart
syn match cFun '[?:]'
syn match cDot contained ',' 
hi cDot gui=italic guifg=#ffa0a0


hi cAOp guifg=khaki gui=italic
hi def link cFun cAOp
hi def link cStructure	cAOp
hi def link cType cAOp
hi def link cStorageClass cAOp
